...
for dataset in 'whole' 'selective'; do
    ...
    for combination in "${combinations[@]}"; do
	    ...
	    timestamp=$(date +%d-%m-%Y_%H-%M-%S)
            ...
            cp /home/breuter/MalariaVaccineEfficacyPrediction/bin/rgscv_multitask_r.py . || { echo "cp /home/breuter/MalariaVaccineEfficacyPrediction/bin/rgscv_multitask.py . failed"; exit 1; }
            python -u rgscv_multitask_r.py --analysis-dir "${ana_dir}" --data-dir "${data_dir}" --combination "${combination}" --identifier "${identifier}" 1> "${out}" 2> "${err}"
        done
    done
done

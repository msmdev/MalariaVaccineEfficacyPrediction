...
for dataset in 'whole' 'selective'; do
    ...
    cp /home/breuter/MalariaVaccineEfficacyPrediction/bin/rncv_RLR_cf.py . || { echo "cp /home/breuter/MalariaVaccineEfficacyPrediction/bin/rncv_RLR_cf.py . failed"; exit 1; }
    python -u rncv_RLR_cf.py --analysis-dir "${ana_dir}" --data-file "${data_dir}/preprocessed_${dataset}_data.csv" --identifier "${dataset}" 1> "${out}" 2> "${err}"

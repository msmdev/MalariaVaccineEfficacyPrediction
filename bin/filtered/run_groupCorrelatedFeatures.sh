...
for dataset in 'whole' 'selective'; do
    ...
    for timepoint in 'all' 'III14' 'C-1' 'C28'; do
        ...
        for threshold in '0.95' '0.98' '0.99'; do
            ...
            python -u groupCorrelatedFeatures.py --data-dir "${data_dir}" --data-file "preprocessed_${dataset}_data.csv" --out-dir "$ana_dir" --timepoint "$timepoint" --correlation_threshold "$threshold" --correlation_method "$method" 1> "${out}" 2> "${err}"

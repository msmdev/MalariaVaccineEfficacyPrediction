    ...
    for method in 'RLR' 'multitaskSVM' 'RF' 'SVM'; do
        ...
        for dataset in 'whole' 'selective'; do
            ...
            if [ "$method" = 'multitaskSVM' ]; then
                ...
                qsub -v "ANA_DIR"="${ana_dir}","DATA_DIR"="${data_dir}","DATA_FILE_ID"="preprocessed_${dataset}_data_spearman_filtered_threshold${threshold}","METHOD"="${method}","NJOBS"=28,"KERNEL_DIR"="$kernel_dir","IDENTIFIER"="$identifier" -N "$jobname" -q short -l walltime=48:00:00,mem=125gb,nodes=1:ppn=28 "run_rncv_BINAC.sh"
                ...
                cp "${topdir}/source/${method}_config.py" . || { echo "cp ${topdir}/source/${method}_config.py . failed"; exit 1; }
                if [ "$method" = 'RLR' ]; then
                    qsub -v "ANA_DIR"="${ana_dir}","DATA_DIR"="${data_dir}","DATA_FILE_ID"="preprocessed_${dataset}_data_spearman_filtered_threshold${threshold}","METHOD"="${method}","NJOBS"=28 -N "$jobname" -q short -l walltime=48:00:00,mem=125gb,nodes=1:ppn=28 "run_rncv_BINAC.sh"
                else
                    qsub -v "ANA_DIR"="${ana_dir}","DATA_DIR"="${data_dir}","DATA_FILE_ID"="preprocessed_${dataset}_data_spearman_filtered_threshold${threshold}","METHOD"="${method}","NJOBS"=4 -N "$jobname" -q short -l walltime=48:00:00,mem=17gb,nodes=1:ppn=4 "run_rncv_BINAC.sh"
                fi
            fi

    ...
    for method in 'multitaskSVM' 'RF' 'RLR' 'SVM'; do
        ...
        for dataset in 'whole' 'selective'; do
            ...
            if [ "$method" = 'multitaskSVM' ]; then
                ...
                cp "${topdir}/bin/run_rgscv_BINAC.sh" . || { echo "cp ${topdir}/bin/run_rgscv_BINAC.sh . failed"; exit 1; }
                cp "${topdir}/source/${method}_config.py" . || { echo "cp ${topdir}/source/${method}_config.py . failed"; exit 1; }
                qsub -v "ANA_DIR"="${ana_dir}","DATA_DIR"="${data_dir}","DATA_FILE_ID"="preprocessed_${dataset}_data_spearman_filtered_threshold${threshold}","METHOD"="${method}","NJOBS"=4 -N "$jobname" -q short -l walltime=24:00:00,mem=17gb,nodes=1:ppn=4 "run_rgscv_BINAC.sh"
            fi

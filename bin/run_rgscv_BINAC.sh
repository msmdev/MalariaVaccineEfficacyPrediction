...
if [ "$METHOD" = 'multitaskSVM' ]; then
    ...
    for combination in "${combinations[@]}"; do
        ...
        python -u rgscv.py --analysis-dir "$ana_dir" --data-dir "$DATA_DIR" --data-file-id "$DATA_FILE_ID" --method "$METHOD" --njobs="$NJOBS" --kernel-dir "$TMPDIR" --combination "$combination" --identifier "$IDENTIFIER" 1> "${out}" 2> "${err}"
    done
else
